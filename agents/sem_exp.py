...
from constants import color_palette, mp_categories_mapping
...
class Sem_Exp_Env_Agent(ObjectGoal_Env21):
    ...
    def plan_act_and_preprocess(
        self, planner_inputs: Dict[str, Any]
    ) -> Tuple[np.ndarray, Dict, bool, Dict[str, Any]]:
        ...
        if planner_inputs["new_goal"]:
            ...
            if (
                np.sum(goal == 1) == 1
                and self.args.task_config == "tasks/objectnav_gibson.yaml"
            ):
        ...
        if action >= 0:
            ...
            if done and not self.info["success"]:
            ...
            self.obs = obs
            self.info = info
            self.info["eve_angle"] = self.eve_angle
            ...
            return obs, self.fail_case, done, self.info
    ...
    def _plan(self, planner_inputs):
        ...
        # channel 0-2: agent global pose
        # channel 3-6: local map boundries
    ...
    def _get_stg(self, grid, start, goal, planning_window):
        ...
        traversible[
            cv2.dilate(self.visited_vis[gx1:gx2, gy1:gy2][x1:x2, y1:y2], self.kernel)
            == 1
        ] = 1
    ...
    def _preprocess_obs(self, obs: np.ndarray, use_seg=True) -> np.ndarray:
        args = self.args
        # print("obs: ", obs)
    ...
    def _preprocess_depth(
        self, depth: np.ndarray, min_d: float, max_d: float
    ) -> np.ndarray:
        depth = depth[:, :, 0] * 1
