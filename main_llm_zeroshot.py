...
def main():
    ...
    # make stairs non-traversible
    local_map[:, 0, :, :][local_map[:, 18, :, :] > 0] = 0
    ...
    for step in range(args.num_training_frames // args.num_processes + 1):
        ...
        for e in range(num_scenes):
            # update agent location on local map
            r, c = locs[e, 1], locs[e, 0]
            loc_r, loc_c = [
                int(r * 100.0 / args.map_resolution),
                int(c * 100.0 / args.map_resolution),
            ...
            if args.eval:
                ...
                if infos[e]["clear_flag"] or local_map[e, 18, loc_r, loc_c] > 0.5:
                    ...
                    # TODO: Run another experiment with the following block commented
                    # if stair_flag[e]:
                    #
                    #     # must > 0
        ...
        if l_step == args.num_local_steps - 1:
            ...
            for e in range(num_scenes):
                ...
                # clear_flag = true indicates that the agent have too many collisions
                # then clear the local map for reconstruction
            ...
            for e in range(num_scenes):
                ...
                # compute frontier region cost, select top 4
                target_edge_map[e], target_point_map[e], Goal_score = (
                ...
                for lay in range(tpm):
                    ...
                    if len(objs_list) > 0 and found_goal[e] == 0:
                        ref_dist = F.softmax(construct_dist(objs_list), dim=0).to(
                            device
                        )
                        new_dist = ref_dist
                        ...
                        frontier_score_list[e].append(
                            new_dist[category_to_id.index(cname)]
                        )
                    else:
                        frontier_score_list[e].append(
    ...
    if args.eval:
        ...
        with open(
            "{}/{}_success_per_cat_pred_thr.json".format(dump_dir, args.split), "w"
        ) as f:
            json.dump(success_per_category, f)

        with open(
            "{}/{}_clusters_per_episode.json".format(dump_dir, args.split), "w"
        ) as f:
            json.dump(visualizaiton_cluster, f)


def draw_map(map, filename):
    h, w = map.shape
